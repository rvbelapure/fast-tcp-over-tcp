...
main (int argc, char *argv[])
{
     ...
     sock_descriptor_t *new_sd;
     pthread_t app_thread;
     while (1) {

         new_sd = gt_accept(sd, (struct sockaddr *)&cad, &alen);
	 if(new_sd == NULL)
	 {
	    fprintf(stderr, "accept failed\n");
            exit (1);
      	 }
	 pthread_create(&app_thread, NULL, serverthread, new_sd);
     }
     gt_close(sd);
}
...
void * serverthread(void * parm)
{
   ...
   ssize_t read = gt_recv(tsd, output_buf, 1024, 0);
   output_buf[read] = '\0';
